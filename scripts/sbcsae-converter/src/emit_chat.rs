...
// ── Document-based CHAT emitter ─────────────────────────────────────────────

use crate::intermediate::{
    BracketRole, ContentElement, OverlapAssignment,
...
/// This is the new pipeline: doc → assignment → CHAT.
pub fn emit_chat_from_doc(
    ...
    for utt in &sorted {
        ...
        let content = emit_doc_content(&utt.elements, &doc.brackets, assignment);
        let terminator = emit_doc_terminator(&utt.terminator);
        let bullet = emit_bullet(utt.start_ms, utt.end_ms);
...
fn emit_doc_content(
    ...
    brackets: &[crate::intermediate::BracketRef],
    assignment: &OverlapAssignment,
) -> String {
    let mut out = String::new();
    let mut need_space = false;
    ...
    for elem in elements {
        let (text, space_before, space_after) = emit_doc_element(elem, brackets, assignment);
        if text.is_empty() {
            continue;
        }
        if need_space && space_before {
    ...
    match elem {
        ...
        ContentElement::Bracket(bracket_id) => {
            if let Some(role) = assignment.roles.get(bracket_id) {
                ...
                // No role assigned — emit as unclassified marker.
                ("⌈?".to_string(), true, true)
            }
        }
...
fn emit_doc_terminator(term: &Terminator) -> &'static str {
    match term {
        Terminator::Period => ".",
        Terminator::Question => "?",
